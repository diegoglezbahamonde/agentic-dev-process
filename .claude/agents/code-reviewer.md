...
- **Use case bypassing the executor**: an HTTP route or worker calling a use case directly instead of `executor.execute(...)`. See [use-case-executor](../../docs/architecture/use-case-executor.md).
- **Constructor-injected use case**: any use case taking ports as constructor args instead of a `ServiceLocator`. See [service-locator-injection](../../docs/architecture/service-locator-injection.md).
- **Reads loading aggregates**: a list/search/report endpoint constructing domain aggregates instead of projecting to DTOs. See [cqrs-read-write-separation](../../docs/architecture/cqrs-read-write-separation.md).
