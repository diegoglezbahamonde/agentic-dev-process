...
- **Honor the conventions you read.** Use cases inherit from `UseCase[Input, Output]` and use the hybrid pattern (ports + executor as optional constructor args defaulting to `ServiceLocator.find(<Port>.__name__)`). Commands and queries both go through `executor.execute(UseCaseClass, input)` — no separate query-handler concept. No `@dataclass` without `frozen=True, slots=True` for commands and DTOs. No business logic in `__init__` (constructor wiring is fine; domain logic is not) or in DB defaults. If the ticket asks for something that violates these, name the violation in **Risks & open questions** and propose a compliant alternative.
